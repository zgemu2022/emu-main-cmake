...
void saveval(pconf *ppconf, int i, int val, char *strval)
{
	...
	if (strcmp(conf_tab[i].name, "firmid") == 0)
	{ // 0
		ppconf->firm_id = val;
	}
	else if (strcmp(conf_tab[i].name, "lcd_server_ip1") == 0)
	{ // 4
		sprintf(&ppconf->lcd_server_ip[0][0], "%s", strval);
	}
	else if (strcmp(conf_tab[i].name, "lcd_server_port1") == 0)
	{ // 5
		ppconf->lcd_server_port[0] = val;
	}
	else if (strcmp(conf_tab[i].name, "lcd_server_ip2") == 0)
	{ // 4
		sprintf(&ppconf->lcd_server_ip[1][0], "%s", strval);
	}
	else if (strcmp(conf_tab[i].name, "lcd_server_port2") == 0)
	{ // 5
		ppconf->lcd_server_port[1] = val;
	}
	else if (strcmp(conf_tab[i].name, "lcd_server_ip3") == 0)
	{ // 4
		sprintf(&ppconf->lcd_server_ip[2][0], "%s", strval);
	}
	else if (strcmp(conf_tab[i].name, "lcd_server_port3") == 0)
	{ // 5
		ppconf->lcd_server_port[2] = val;
	}
	else if (strcmp(conf_tab[i].name, "lcd_server_ip4") == 0)
	{ // 4
		sprintf(&ppconf->lcd_server_ip[3][0], "%s", strval);
	}
	else if (strcmp(conf_tab[i].name, "lcd_server_port4") == 0)
	{ // 5
		ppconf->lcd_server_port[3] = val;
	}
	else if (strcmp(conf_tab[i].name, "lcd_server_ip5") == 0)
	{ // 4
		sprintf(&ppconf->lcd_server_ip[4][0], "%s", strval);
	}
	else if (strcmp(conf_tab[i].name, "lcd_server_port5") == 0)
	{ // 5
		ppconf->lcd_server_port[4] = val;
	}
	else if (strcmp(conf_tab[i].name, "lcd_server_ip6") == 0)
	{ // 4
		sprintf(&ppconf->lcd_server_ip[5][0], "%s", strval);
	}
	else if (strcmp(conf_tab[i].name, "lcd_server_port6") == 0)
	{ // 5
		ppconf->lcd_server_port[5] = val;
	}
	else if (strcmp(conf_tab[i].name, "lcd_num") == 0)
	{ // 1
		ppconf->lcd_num = val;
	}
	...
	else if (strcmp(conf_tab[i].name, "plc_server_ip") == 0)
	{ // 4
		sprintf(&ppconf->plc_server_ip[0], "%s", strval);
	}
	else if (strcmp(conf_tab[i].name, "plc_server_port") == 0)
	{										 // 5
	...
	else if (strcmp(conf_tab[i].name, "LOGENABLE") == 0)
	{															 // 24
		printf("tcu是否支持将协议日志写入flash  =%d\r\n ", val); //
		ppconf->iflog = val;
	}
	else if (strcmp(conf_tab[i].name, "HARDWARE_VER") == 0)
	{											// 25
		printf("tcu硬件版本号  =%d\r\n ", val); //
		ppconf->hardware_ver = val;
	}
	else if (strcmp(conf_tab[i].name, "START_LOG") == 0)
	{ // 27
		char strx[7];
		char b1, b2;
	...
	else if (strcmp(conf_tab[i].name, "LOG_DAYS") == 0)
	{ // 28
		ppconf->logdays = val;
		printf("日志开始记录天数=%d\n", ppconf->logdays);
	}
	else if (strcmp(conf_tab[i].name, "MAIN_VER") == 0)
	{ // 29

		sprintf(ppconf->main_ver, "%s\n", strval);
		printf("主控版本号  =%s\r\n ", ppconf->main_ver); // 主控版本号
	}
	...
	else if (strcmp(conf_tab[i].name, "bams_num") == 0)
	{
		printf("BAMS 数量:%d\n", val); // 2
		ppconf->bams_num = val;
	}
	...
	for (i = 0; i < num; i++)
	{
		...
		if (strcmp(item->string, conf_tab[i].name) == 0)
		{
			...
			if (item->type == cJSON_String && conf_tab[i].typeVal == TY_STR)
			{
				...
				{
					if (strcmp(item->string, "START_LOG") && strcmp(item->string, "MAIN_VER"))
					{
			...
			else if ((item->type == cJSON_Number) && (conf_tab[i].typeVal == TY_INT))
			{
				...
				else if (strcmp(item->string, "LOGENABLE") && atoi(conf_tab[i].val) != item->valueint)
				{
